...
- **Module Permissions**:
  ...
  - **Dev Member Project & Task Scoping**:
    ...
    - **Leave/WFH approval moved to managers**: HR no longer has Approve/Reject on the Leave Requests tab (or the Attendance tab's WFH list) — those views are read-only for HR now (status, reason, and once decided, who approved/rejected and their note). Approval/rejection happens through the employee's manager via Manager Hub. A "Leave/WFH Submitted" notification goes to that specific manager (resolved via `EmployeeRepository.find_by_exact_name`), not broadcast to every Owner-department/HR person — it only falls back to an "owner" broadcast if the employee has no manager on file.
      - **Single named-HR exception (UI-only)**: the account matching `HR_APPROVER_EMAIL` in `frontend-next/src/app/hr/page.tsx` also gets Approve/Reject on this screen, from the leave drawer (which both the Leave Requests tab and the Attendance tab's WFH list open), using the same note/reason modal Manager Hub uses. **Nothing in the backend or in any access level changed to allow this** — `leave_service.approve_leave`/`reject_leave` and `wfh_request_service.approve_request`/`reject_request` have always gated on `has_role(persona, "hr", "owner") or is_manager`, so any holder of the `hr` access level was already permitted by the API; only the buttons were missing. That means this gate stops nobody at the API level — it is a UI convenience for one person, not a security boundary. To make it role-wide instead, swap the email comparison for the `hr` access check that the backend already enforces.
...
- **RAG assistant** (`POST /api/policies/ask`, `app/services/groq_service.py` + `policy_service.py`): deliberately **not** a vector-embedding pipeline — every question re-reads all current policies fresh from the DB and stuffs their content into the prompt, so anything an Owner just published or edited is immediately answerable with zero re-indexing step. Backed by Groq's OpenAI-compatible chat completions API (`GROQ_API` env var, model configurable via `GROQ_MODEL`, defaults to `openai/gpt-oss-120b`). If `GROQ_API` is unset, the endpoint returns a clean "not configured" message rather than erroring. **Groq retires models without notice** — the previous default (`llama-3.3-70b-versatile`) was decommissioned and every question started failing with a bare 502, because `groq_service` used to discard Groq's response body and collapse every cause (revoked key, dead model, rate limit) into one indistinguishable error. It now logs the real status/code/body under the `orbit.groq` logger and returns an actionable message per case, so "is it the key or the model?" is answerable from the logs alone. To check what's currently available: `GET https://api.groq.com/openai/v1/models` with the same bearer token.
